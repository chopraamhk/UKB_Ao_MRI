...
def conv2d_bn_relu(x, filters, training, kernel_size=3, strides=1):
    ...
    x_relu = tf.compat.v1.nn.relu(x_bn)
    return x_relu
...
def residual_unit(x, filters, training, strides=1):
    """
        ...
        He et al. Identity .ngs in Deep Residual Networks, ECCV 2016.
    ...
    # Use projection for increased dimension and identity .ng
...
def bottleneck_unit(x, filters, training, strides=1):
    """
        ...
        We also move batch_norm and relu before conv, as shown in He's next paper
          He at al. Identity .ngs in Deep Residual Networks, ECCV 2016.
          https://arxiv.org/pdf/1603.05027.
        """
    orig_x = x
    with tf.compat.v1.name_scope('sub1'):
...
def transpose_upsample2d(x, factor, constant=True):
    ...
    output_shape = tf.compat.v1.stack([x_shape[0], x_shape[1] * factor, x_shape[2] * factor, x.shape[3].value])
    ...
    if constant:
        filt = tf.compat.v1.constant(filt_val, dtype=tf.compat.v1.float32)
    else:
        filt = tf.compat.v1.Variable(filt_val, dtype=tf.compat.v1.float32)
    ...
    # a problem for the next step tf.compat.v1.concat(), which requires a known shape.
    ...
    x_up = tf.compat.v1.nn.conv2d_transpose(x, filter=filt, output_shape=output_shape,
                                  strides=[1, factor, factor, 1], padding='SAME')
    x_out = tf.compat.v1.reshape(x_up,
...
def build_FCN(image, n_class, n_level, n_filter, n_block, training, same_dim=32, fc=64):
    ...
    with tf.compat.v1.name_scope('concat'):
        ...
        net['concat'] = tf.compat.v1.concat(list_up, axis=-1)
...
def build_ResNet(image, n_class, n_level, n_filter, n_block, training,
    ...
    with tf.compat.v1.name_scope('out'):
        ...
        # tf.compat.v1.nn.softmax_cross_entropy() accepts the unscaled logits and performs softmax
