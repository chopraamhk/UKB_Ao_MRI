    ...
    with tf.compat.v1.variable_scope('UNet'):
        ...
        for l in range(n_level - 2, -1, -1):
            with tf.compat.v1.variable_scope('conv{}_up'.format(l)):
                ...
                x = tf.compat.v1.concat([net['conv{}'.format(l)], x], axis=-1)
        ...
        with tf.compat.v1.variable_scope('conv_out'):
            ...
            # function tf.compat.v1.nn.softmax_cross_entropy() accepts the unscaled logits
...
def focal_loss(labels, logits, n_class, alpha):
    ...
    # label_1hot: NXYC
    # logits: NXYC
    label_1hot = tf.compat.v1.one_hot(indices=labels, depth=n_class)

    # alpha_t: NXYC
    background = tf.compat.v1.equal(labels, 0)
    foreground = tf.compat.v1.not_equal(labels, 0)
    alpha_t = alpha * tf.compat.v1.cast(foreground, tf.compat.v1.float32) + (1 - alpha) * tf.compat.v1.cast(background, tf.compat.v1.float32)
    alpha_t = tf.compat.v1.expand_dims(alpha_t, axis=-1)

    # Multiply alpha_t with label_1hot first, then use
    # softmax_cross_entropy_with_logits
    label_1hot = alpha_t * label_1hot
    label_loss = tf.compat.v1.nn.softmax_cross_entropy_with_logits(labels=label_1hot, logits=logits)
    loss = tf.compat.v1.reduce_mean(label_loss)
    return loss
...
def UNet_Model(images, labels, n_class, n_level, n_filter, n_block, training):
    ...
    # prob: NXYC
    # pred: NXY
    prob = tf.compat.v1.nn.softmax(logits, name='prob')
    pred = tf.compat.v1.cast(tf.compat.v1.argmax(prob, axis=-1), dtype=tf.compat.v1.int32, name='pred')
...
def Temporal_UNet_Model(images, labels, n_class, n_level, n_filter, n_block, n_step,
    ...
    for t in range(n_step):
        # logits_fr: NXYC
        logits_fr = logits[:, t]
        label_1hot = tf.compat.v1.one_hot(indices=labels[:, t], depth=n_class)
        label_loss = tf.compat.v1.nn.softmax_cross_entropy_with_logits(labels=label_1hot, logits=logits_fr)
        ...
        loss_fr = w * tf.compat.v1.reduce_mean(label_loss)
        loss += [loss_fr]

    # Average loss across time frames
    loss = tf.compat.v1.reduce_sum(tf.compat.v1.stack(loss, axis=0)) / sum_w
...
def Conv_LSTM(features, lstm_input_shape, n_hidden, n_step, n_class):
    ...
    with tf.compat.v1.variable_scope('LSTM'):
        cell = tf.compat.v1.contrib.rnn.Conv2DLSTMCell(input_shape=lstm_input_shape, output_channels=n_hidden,
                                             kernel_shape=[3, 3])

        initial_state = cell.zero_state(tf.compat.v1.shape(features)[0], tf.compat.v1.float32)
        state = initial_state
        ...
        #
        # inputs = tf.compat.v1.unstack(inputs, num=num_steps, axis=1)
        # outputs, state = tf.compat.v1.contrib.rnn.static_rnn(cell, inputs, initial_state=self._initial_state)
        outputs = []
        for t in range(n_step):
            if t > 0:
            ...
            outputs.nd(logits)
        # outputs: NTXYC
        outputs = tf.compat.v1.stack(outputs, axis=1)
    return outputs
...
def BiConv_LSTM(features, lstm_input_shape, n_hidden, n_step, n_class):
    ...
    with tf.compat.v1.variable_scope('LSTM'):
        ...
        #
        # inputs = tf.compat.v1.unstack(inputs, num=num_steps, axis=1)
        # outputs, state = tf.compat.v1.contrib.rnn.static_rnn(cell, inputs,
        #                            initial_state=self._initial_state)
        # Forward LSTM cell
        with tf.compat.v1.variable_scope('forward'):
            cell_fw = tf.compat.v1.contrib.rnn.Conv2DLSTMCell(input_shape=lstm_input_shape, output_channels=n_hidden,
                                                    kernel_shape=[3, 3])
            state_fw = cell_fw.zero_state(tf.compat.v1.shape(features)[0], tf.compat.v1.float32)
            cell_outputs_fw = []
        ...
        with tf.compat.v1.variable_scope('backward'):
            cell_bw = tf.compat.v1.contrib.rnn.Conv2DLSTMCell(input_shape=lstm_input_shape, output_channels=n_hidden,
                                                    kernel_shape=[3, 3])
            state_bw = cell_bw.zero_state(tf.compat.v1.shape(features)[0], tf.compat.v1.float32)
            cell_outputs_bw = []
        ...
        with tf.compat.v1.variable_scope('output'):
            ...
            for t in range(n_step):
                ...
                # cell_output: NXYC
                cell_output = tf.compat.v1.concat([cell_outputs_fw[t], cell_outputs_bw[n_step - 1 - t]], axis=-1)
...
def UNet_LSTM_Model(images, labels, n_class, n_level, n_filter, n_block,
    ...
    images_shape = tf.compat.v1.shape(images)
    images = tf.compat.v1.reshape(images, [-1, images_shape[2], images_shape[3], images.shape[4]])
    ...
    # features: NTXYC
    features = tf.compat.v1.reshape(features, [images_shape[0], images_shape[1], images_shape[2], images_shape[3], n_filter[0]])
    ...
    if seq2seq:
        ...
        label_1hot = tf.compat.v1.one_hot(indices=labels[:, t_anno], depth=n_class)
        label_loss = tf.compat.v1.nn.softmax_cross_entropy_with_logits(labels=label_1hot, logits=logits_fr)
        loss = tf.compat.v1.reduce_mean(label_loss)
    ...
    # prob: NTXYC
    # pred: NTXY
    prob = tf.compat.v1.nn.softmax(outputs, name='prob')
    pred = tf.compat.v1.cast(tf.compat.v1.argmax(prob, axis=-1), dtype=tf.compat.v1.int32, name='pred')
    return loss, prob, pred
