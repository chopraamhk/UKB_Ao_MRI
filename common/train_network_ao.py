...
tf.compat.v1.flags.DEFINE_integer('time_window', 11,
                            'Time window for LSTM.')
tf.compat.v1.flags.DEFINE_integer('train_batch_size', 5,
                            'Number of images for each training batch.')
tf.compat.v1.flags.DEFINE_integer('validation_batch_size', 5,
...
tf.compat.v1.flags.DEFINE_integer('num_level', 5,
                            'Number of network levels.')
tf.compat.v1.flags.DEFINE_integer('num_hidden', 16,
                            'Number of hidden status dimension in LSTM.')
tf.compat.v1.flags.DEFINE_integer('train_iteration', 20000,
                            'Number of training iterations.')
tf.compat.v1.flags.DEFINE_float('learning_rate', 1e-3,
                          'Learning rate.')
tf.compat.v1.flags._global_parser.add_argument('--reduce_lr_after', action=.nd',
                                         help='Reduce the learning rate after this many iterations.')
tf.compat.v1.flags.DEFINE_enum('seq_name', 'ao', ['ao'],
                         'Sequence name.')
tf.compat.v1.flags.DEFINE_enum('model', 'UNet', ['UNet', 'UNet-LSTM', 'Temporal-UNet'],
                         'Model name.')
...
tf.compat.v1.flags.DEFINE_string('log_dir', '/vol/bitbucket/wbai/ukbb_cardiac_ao/log',
                           'Directory for saving the log file.')
tf.compat.v1.flags.DEFINE_string('checkpoint_dir', '/vol/bitbucket/wbai/ukbb_cardiac_ao/model',
                           'Directory for saving the trained model.')
tf.compat.v1.flags.DEFINE_string('model_path', '',
                           'Path to the saved trained model.')
tf.compat.v1.flags.DEFINE_boolean('z_score', True,
                            'Normalise the image intensity to z-score. Otherwise, rescale the intensity.')
tf.compat.v1.flags.DEFINE_boolean('bidirectional', True,
                            'Bi-directional LSTM.')
tf.compat.v1.flags.DEFINE_boolean('seq2seq', True,
                            'Sequence to sequence learning. Otherwise, only learn the last time frame.')
tf.compat.v1.flags.DEFINE_integer('weight_R', 5,
                            'Radius of the weighting window.')
tf.compat.v1.flags.DEFINE_float('weight_r', 0,
                          'Power of weight for the seq2seq loss. 0: uniform; 1: linear; 2: square.')
tf.compat.v1.flags.DEFINE_boolean('joint_train', False,
                            'Joint training of UNet and LSTM.')
tf.compat.v1.flags.DEFINE_boolean('from_scratch', False,
...
def main(argv=None):
    ...
    # Use int32 for label_pl as tf.compat.v1.one_hot uses int32
    if FLAGS.model == 'UNet':
        # image_pl: NXYC
        # label_pl: NXY
        image_pl = tf.compat.v1.placeholder(tf.compat.v1.float32, shape=[None, None, None, 1], name='image')
        label_pl = tf.compat.v1.placeholder(tf.compat.v1.int32, shape=[None, None, None], name='label')
    elif FLAGS.model == 'UNet-LSTM' or FLAGS.model == 'Temporal-UNet':
        # image_pl: NTXYC
        # label_pl: NTXY
        # mask_pl:  NTXY
        image_pl = tf.compat.v1.placeholder(tf.compat.v1.float32, shape=[None, None, None, None, 1], name='image')
        label_pl = tf.compat.v1.placeholder(tf.compat.v1.int32, shape=[None, None, None, None], name='label')
    else:
    ...
    # properly.
    training_pl = tf.compat.v1.placeholder(tf.compat.v1.bool, shape=[], name='training')
    ...
    accuracy = tf.compat.v1.categorical_accuracy(pred_fr, label_fr)
    dice_aa = tf.compat.v1.categorical_dice(pred_fr, label_fr, 1)
    dice_da = tf.compat.v1.categorical_dice(pred_fr, label_fr, 2)
