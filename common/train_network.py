...
from ukbb_cardiac.common.image_utils import tf.compat.v1.categorical_accuracy, tf.compat.v1.categorical_dice
...
tf.compat.v1.flags.DEFINE_integer('train_batch_size', 2,
                            'Number of images for each training batch.')
tf.compat.v1.flags.DEFINE_integer('validation_batch_size', 2,
                            'Number of images for each validation batch.')
tf.compat.v1.flags.DEFINE_integer('train_iteration', 50000,
...
tf.compat.v1.flags.DEFINE_string('log_dir',
...
def get_random_batch(filename_list, batch_size, image_size=192, data_augmentation=False,
    ...
    while n_selected < batch_size:
        ...
        if os.path.exists(image_name) and os.path.exists(label_name):
            ...
            # Use list for.nding, which is much faster than numpy array
            for z in range(Z):
                images += [image[:, :, z]]
                labels += [label[:, :, z]]
...
def main(argv=None):
    ...
    # Loss
    label_1hot = tf.compat.v1.one_hot(indices=label_pl, depth=n_class)
    ...
    # Evaluation metrics
    accuracy = tf.compat.v1.categorical_accuracy(pred, label_pl)
    dice_lv = tf.compat.v1.categorical_dice(pred, label_pl, 1)
    dice_myo = tf.compat.v1.categorical_dice(pred, label_pl, 2)
    dice_rv = tf.compat.v1.categorical_dice(pred, label_pl, 3)
    dice_la = tf.compat.v1.categorical_dice(pred, label_pl, 1)
    dice_ra = tf.compat.v1.categorical_dice(pred, label_pl, 2)
